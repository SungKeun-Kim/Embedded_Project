...
| PD0  | GPIO_Output | CLK_LCD — 74HCT574 #1 래치 클럭 (LCD용) | LQFP80 전용 핀  |
| PD1  | GPIO_Output | CLK_LED — 74HCT574 #2 래치 클럭 (LED용) | LQFP80 전용 핀  |
...
> PD2, PB0~PB2: **v4 해제 핀** (구 LCD_D4, LED_NORMAL/HL_SET/8POWER).
...
| PC5 | GPIO_Input | BTN_FREQ (짧게: 주파수 5초 표시, 5초 장기: FREQ SET 모드) |

> 버튼은 디스플레이 보드 내 외부 10kΩ 풀업 저항 사용 (3.3V ← R_PU ← 핀). gpio_init.c: GPIO_NOPULL.

### LED 표시등 (6개, 74HCT574 #2 Q 출력, 560Ω 저항)

LED는 74HCT574 #2 Q0~Q5에 연결. DATA 버스(PB10~PB15)에 마스크 설정 후 CLK_LED(PD1) 상승엣지로 래치.
...
| PC13 | GPIO_Output | —   | RS-485 DE (Driver Enable, HIGH=송신)   |
| PC14 | GPIO_Output | —   | RS-485 /RE (Receiver Enable, LOW=수신) |

> **핀 재배치 (v2)**: RS-485 DE/RE를 PA4/PA5에서 PC13/PC14로 이동.
...
| 핀   | 포트       | 기능                                                 |
| ---- | ---------- | ---------------------------------------------------- |
| PC6  | GPIO_Input | REMOTE 입력 (SHORT=발진ON, OPEN=발진OFF, 4-COM 겸용) |
| PC10 | GPIO_Input | 8POWER BCD BIT1                                      |
| PC11 | GPIO_Input | 8POWER BCD BIT2                                      |
| PC12 | GPIO_Input | 8POWER BCD BIT3                                      |

> SENSOR 기능은 알람 릴레이로 구현 예정. PC7은 LC_RELAY3으로 재할당 (v3).
...
| 핀   | 포트        | 기능                                           |
| ---- | ----------- | ---------------------------------------------- |
| PC9  | GPIO_Output | LC_RELAY1 (인덕턴스 L1 스위칭, PB8→PC9 재배치) |
| PC15 | GPIO_Output | LC_RELAY2 (인덕턴스 L2 스위칭)                 |
| PC7  | GPIO_Output | LC_RELAY3 (인덕턴스 L3 스위칭)                 |
| PC8  | GPIO_Output | LC_RELAY4 (인덕턴스 L4 스위칭)                 |

> 릴레이 ON/OFF 조합으로 LC 탱크 인덕턴스 값 변경 → 공진주파수 미세 조정.
> ULN2003A #2: LC 릴레이 4개 사용 (3채널 여유).
> **v4 변경**: PB8 → PC9 재배치. LC 릴레이 4개 모두 **GPIOC (PC7/8/9/15)로 통합**.
...
> **v3 변경**: PC7→LC_RELAY3, PC8→LC_RELAY4로 전환. 여유 핀 5→3개로 감소.
> **v4 변경**: PB8→PC9 재배치 + 74HCT574 방법 C 전환으로 PB0~PB2, PD2 해제 → 여유 핀 3→7개로 증가.
...
| 포트     | 사용 핀 수 | 할당 목록                                                          |
| -------- | ---------- | ------------------------------------------------------------------ |
| **PA**   | 13         | PA0~PA4, PA6~PA15 (PA5 여유)                                       |
| **PB**   | 9          | PB3~PB5, PB10~PB15 (PB0/1/2/8/9 여유)                              |
| **PC**   | 14         | PC0~PC12, PC15 (LC릴레이 4개 모두 GPIOC로 통합)                    |
| **PD**   | 2          | PD0(CLK_LCD), PD1(CLK_LED) (PD2 여유)                              |
| **PF**   | 2          | PF0(OSC_IN), PF1(OSC_OUT) — **HSE 크리스탈 전용**                  |
| **총계** | **40/51**  | GPIO 40개 + HSE 2개 사용, **여유 핀 7개** (PA5, PB0~2, PB8/9, PD2) |
...
```text
  ...
  버튼 6ea    │          LQFP80                          │  LC_REL3/4(PC7,PC8)
...
| **우측**      | 알람 릴레이 5개 + LC 릴레이 4개                                | PA11,PA12,PA15,PB3,PB4 / PC9,PC7,PC8,PC15 |
...
- **LCD 데이터 핀 혼재**: PD0~PD2(하측)와 PB12/PB13/PB15(하측)가 물리적으로 분산되어 있으므로, LCD FPC 커넥터를 하측 중앙에 배치하여 배선 경로 최적화.
