...
예시:
  ...
  git commit -m "Phase 2: LCD1602 + 6-button + 8-LED + menu FSM"
...
1. KiCad/Altium 회로도 최종 확인 → `docs/STM32G474MET6_LQFP80_핀맵.md`와 대조
2. STM32CubeIDE에서 **STM32G474MET6** (LQFP80) 선택
3. `.ioc` 핀 할당 (`핀맵.md` 참조):

   | 기능 그룹 | 핀 | CubeIDE 설정 |
   |----------|-----|-------------|
   | HRTIM PWM | PA8, PA9 | HRTIM1_CHA1/CHA2 (AF13) |
   | LCD | PD0~PD2, PB12, PB13, PB15 | GPIO_Output |
   | 버튼 6ea | PC0~PC5 | GPIO_Input (Pull-up) |
   | LED 6ea | PB0~PB2, PB10~PB11, PB14 | GPIO_Output |
   | ADC | PA0, PA1, PA6, PA7 | ADC1_IN1/IN2, ADC2_IN3/IN4 |
   | RS-485 | PA2~PA3(USART2) + PC13,PC14(DE/RE) | USART2 + GPIO |
   | 외부제어 | PC6, PC10~PC12 | GPIO_Input (Pull-up) |
   | 알람릴레이 6ea | PA11, PA12, PA15, PB3, PB4, PA10 | GPIO_Output |
   | LC릴레이 4ea | PB8, PC15, PC7, PC8 | GPIO_Output |
   | 부저 | PB5 | GPIO_Output |
...
copilot-instructions.md의 핀맵 섹션과 docs/STM32G474MET6_LQFP80_핀맵.md를 읽어라.
...
5) 알람 임계값 기본값
6) Modbus 기본 슬레이브 주소
7) 버튼 인덱스 enum (BTN_START_STOP=0, ..., BTN_FREQ=5)
8) LED 인덱스 enum
...
- [ ] 버튼 6개 GPIO 읽기 (풀업 상태에서 HIGH, 누르면 LOW)
...
⚠️ config.h의 LCD 핀 정의를 반드시 사용: LCD_D5=PB12, D6=PB13, D7=PB15 (LQFP80 재배치)

[2회차] button.h/c
include/config.h의 BTN_xxx 정의를 사용하여 6개 버튼 디바운싱 구현.
...
- [ ] 6개 버튼 단일/장기/반복 이벤트 감지
- [ ] MODE 순환: NORMAL→H/L SET→8POWER→NORMAL
- [ ] 6개 LED 모드별 점등/깜빡임

**커밋:** `git commit -m "Phase 2: LCD1602 + 6-button + 6-LED + menu FSM"`
