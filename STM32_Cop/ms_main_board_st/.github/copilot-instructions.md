...
**STM32G474RETx (LQFP64)** 기반 메가소닉(Megasonic) 발진기 메인보드 펌웨어. **CMake + STM32Cube HAL** 프레임워크로 빌드.
HRTIM 고해상도 타이머(184ps 분해능)를 이용한 500kHz~2MHz 메가소닉 PWM 생성,
LCD1602 캐릭터 디스플레이(74HCT574 ×2, 20핀 커넥터), 5개 택트 스위치 + 6개 LED 표시등,
7채널 주파수 선택 (500kHz~2MHz, 250kHz 단위), 8단계 외부 출력 제어, 다중 알람 릴레이 출력,
**자동 공진 탐색** (주파수 7채널 × LC 릴레이 16조합 = 최대 112 포인트 VSWR 스윕으로 트랜스듀서 공진점 자동 검출),
...
- 타겟: STM32G474RETx (Cortex-M4, 170 MHz, Flash 512 KB, SRAM 128 KB, FPU, **HRTIM**, COMP×7, OpAmp×4)
- 보드: 커스텀 보드 LQFP64 (외부 HSE 8MHz 크리스탈 사용, ±20ppm 정밀도)
...
| 공진 탐색  | `megasonic_ctrl` 내 `ScanResonance()` + LC 릴레이 조합 제어               | 트랜스듀서 C₀ 미지 상태에서 자동 탐색  |
...
STM32G474RET6 신규 설계에서의 대체/유지/추가 방안을 정리한다.

#### 제거 대상 (STM32G474 내장 주변장치로 대체)

| 기존 IC        | 기능                                        | 대체 수단                                                              | 비고                                                                                                 |
| -------------- | ------------------------------------------- | ---------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| **PIC16F873A** | 8비트 MCU (제어 중추)                       | **STM32G474RET6**                                                      | Cortex-M4F 170MHz, HRTIM, 512KB Flash, 128KB SRAM. 성능 수백 배 향상                                 |
...
| **74HC574**    | 옥탈 D 플립플롭 래치                        | **불필요**                                                             | PIC의 부족한 GPIO를 확장하기 위한 병렬 버스 래치. STM32G474 LQFP64은 ~51개 GPIO로 충분               |
| **74HC138D**   | 3-to-8 라인 디코더                          | **불필요**                                                             | GPIO 확장/주소 디코딩용. LQFP64 패키지의 GPIO로 직접 연결                                            |
...
| ---------------------------------- | ------------------------------------------------ | --------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| **ULN2003A** (ILN2003AD)           | 달링턴 트랜지스터 어레이 (7채널 릴레이 드라이버) | **ULN2003A ×2 유지** 또는 **개별 N-ch MOSFET (2N7002, BSS138)** | #1: 알람 릴레이 5개 = 5채널 (IN6/IN7 여유 2채널). #2: LC 탱크 릴레이 4개 = 4채널(3채널 여유). 부저는 PC5→KEC105S 직접 구동 |
...
| **절연 게이트 드라이버** | 듀얼 채널 절연 구동               | **UCC21520** (TI, 5.7kVrms 절연, 4A peak, 18ns 지연)                                                 | HRTIM CHA1/CHA2 → INA/INB 입력. 하이사이드(VDDA)는 절연 전원 필수. `docs/circuit_ref_64pin.md` 참조         |
...
| **RS-485 트랜시버**      | Modbus RTU 반이중 통신            | **MAX3485** 또는 **SP3485** (3.3V, 반이중)                                                           | 기존 RS-422(전이중) → RS-485(반이중) 변경. DE/RE 핀: PB0/PB1                                                |
| **BUCK 컨버터**          | 48V→가변 DC 전력 제어             | **LM5005** (TI, SOIC-14, 7~75V, 동기 정류)                                                           | DAC(PA4)로 FB 전압 조절, 0.1W~10W 출력 제어. `docs/circuit_ref_64pin.md` 섹션 9 참조                        |
...
> **회로 상세**: `docs/circuit_ref_64pin.md` 파일에 UCC21520 + IPB200N15N3 + 절연 DC-DC 회로도 및 BOM 수록.

#### IC 수량 비교 요약

| 항목                    | 기존 HS AUP                                          | 신규 STM32G474 설계                                                 |
| ----------------------- | ---------------------------------------------------- | ------------------------------------------------------------------- |
| MCU                     | PIC16F873A ×1                                        | **STM32G474RET6 ×1**                                                |
...
### STM32G474RET6 vs STM32G431RBT6 비교

| 항목         | STM32G431RBT6 (초음파용) | **STM32G474RET6 (메가소닉용)**    |
| ------------ | ------------------------ | --------------------------------- |
...
| Op-Amp       | 3개                      | **4개**                           |
| DAC          | 3채널                    | 3채널                             |
| 패키지       | LQFP64                   | **LQFP64** (충분한 핀 여유)       |
| GPIO         | ~51개                    | **~51개** (64핀 패키지 본딩 기준) |
...
| PC14 | GPIO_Output | CLK_LCD — 74HCT574 #1 래치 클럭 (LCD용) |
| PC15 | GPIO_Output | CLK_LED — 74HCT574 #2 래치 클럭 (LED용) |

> PB10~PB15: 6비트 공유 DATA 버스. CLK_LCD(PC14) 상승엣지로 #1에 {RS,EN,D4~D7} 래치 → LCD1602.
> CLK_LED(PC15) 상승엣지로 #2에 {LED1~6} 래치. 74HCT574 VCC=5V, /OE=GND 고정.
> 디스플레이 보드 분리 구조, 20핀 커넥터로 연결. 버튼 풀업(외부 10kΩ), 부저(KEC105S) 포함.
> PD2는 SENSOR 입력으로 사용, PB0/PB1은 RS-485 DE/RE로 사용.
...
LED는 74HCT574 #2 Q0~Q5에 연결. DATA 버스(PB10~PB15)에 마스크 셋팅 후 CLK_LED(PC15) 상승엣지로 래치.
...
| PB0 | GPIO_Output | RS-485 DE (Driver Enable, HIGH=송신 활성화)   |
| PB1 | GPIO_Output | RS-485 /RE (Receiver Enable, LOW=수신 활성화) |

> 기존 제품의 RS-422 (D-SUB 9핀) 를 RS-485 반이중으로 변경. Modbus RTU 프로토콜 적용.
> **v5 변경**: DE/RE 핀을 PB0/PB1로 이동 (PA4를 DAC로 사용).
...
| 핀   | 포트       | 기능                                                      |
| ---- | ---------- | --------------------------------------------------------- |
| PA15 | GPIO_Input | REMOTE 입력 (SHORT=발진ON, OPEN=발진OFF, 4-COM 단자 겸용) |
...
| PD2  | GPIO_Input | SENSOR 입력 (OPEN=Err7/Err8, SHORT=정상)                  |
...
| PA10 | GPIO_Output | RELAY_LOW (Err1 LOW ALARM 시 SHORT)                                     |
...
> **부저(PC5)**: ULN2003A #1에서 제거 → KEC105S(NPN, 내장 베이스 저항) 직접 구동 (디스플레이 보드 탑재).

#### LC 탱크 인덕턴스 조합 릴레이 (4개, ULN2003A #2로 구동)

| 핀  | 포트        | 기능                                             |
| --- | ----------- | ------------------------------------------------ |
| PC6 | GPIO_Output | LC_RELAY4 (인덕턴스 L4 스위칭, **6.8µH**, bit3)  |
| PC7 | GPIO_Output | LC_RELAY3 (인덕턴스 L3 스위칭, **4.7µH**, bit2)  |
| PC8 | GPIO_Output | LC_RELAY2 (인덕턴스 L2 스위칭, **2.2µH**, bit1)  |
| PC9 | GPIO_Output | LC_RELAY1 (인덕턴스 L1 스위칭, **0.69µH**, bit0) |

> **고정 + 릴레이 조합 인덕턴스 설계 (자동 공진 탐색용)**:
>
> - 고정 인덕턴스: L_BASE = 3.3µH || 4.7µH = **1.94µH** (근사)
> - LC_RELAY1(PC9) = L1 = **0.69µH** (1µH || 2.2µH, bit0)
> - LC_RELAY2(PC8) = L2 = **2.2µH** (bit1)
> - LC_RELAY3(PC7) = L3 = **4.7µH** (bit2)
> - LC_RELAY4(PC6) = L4 = **6.8µH** (bit3)
> - 총합: L_total = L_BASE + 선택 릴레이 합
> - 16가지 조합 범위: **1.94µH(0000) ~ 16.33µH(1111)**
> - 주요 타깃: 2.64µH≈2.63(0001, 2MHz/2.4nF PZT), 16.35µH≈16.33(1111, 1MHz/1.55nF PZT)
...
| 핀  | 포트        | 기능                                                                                     |
| --- | ----------- | ---------------------------------------------------------------------------------------- |
| PC5 | GPIO_Output | KEC105S Base 구동 (디스플레이 보드 내 부저 트랜지스터, GPIO_MODE_OUTPUT_PP, GPIO_NOPULL) |
...
config.h 기준 현재 할당되지 않은 여유 핀. **6개**.

| 핀   | 비고                                             |
| ---- | ------------------------------------------------ |
| PA5  | ADC2_IN13 또는 GPIO (BUCK 보조 ADC 옵션)         |
| PB2  | GPIO (구 LED_8POWER, 74HCT574 #2로 전환 후 해제) |
| PB5  | GPIO (BUZZER PC5 재배치로 해제)                  |
| PB8  | GPIO (확장용, 기존 CLK_LCD 해제)                 |
| PB9  | GPIO (확장용, 기존 CLK_LED 해제)                 |
| PC13 | GPIO (SENSOR PD2 재배치로 해제)                  |

> **GPIO 총계**: FREQ 버튼 삭제 기준 GPIO 41개 사용.
> 비전원 핀 기준으로 GPIO 41 + SWD 2 + HSE 2 + BOOT0 1 + NRST 1 = **47핀 사용**, **여유 6핀**.
> PF0/PF1은 HSE 크리스탈 전용. LC_RELAY1~4는 PC6~PC9로 재배치.
> RS-485 DE/RE는 PB0/PB1 사용, CLK_LCD/CLK_LED는 PC14/PC15 사용, SENSOR는 PD2 사용.

#### 🔧 PCB 아트웍 배선 라우팅 가이드 (LQFP64 물리 핀 기준)

```text
              ┌──── Top (49~64) ───────────────────────┐
              │ PB3~PB4(릴레이H/T) PB6~PB7(디버그UART)     │
              │ BOOT0                                    │
   Left       │                                         │  Right
  (1~16)      │        STM32G474RET6                     │  (33~48)
  버튼 5ea    │          LQFP64                          │  LED W/kHz
  ...
  (PA2~PA5)   │                                         │
              └──── Bottom (17~32) ─────────────────────┘
                DATA버스(PB10~PB15) CLK_LCD(PC14) CLK_LED(PC15)
                20핀 커넥터→디스플레이 보드(LCD+LED+BTN+BUZZER)
                외부제어(PA15,PC10~PC12,PD2)
```
...
| **하측 중앙** | 20핀 커넥터 → 디스플레이 보드 (74HCT574×2, LCD+LED+BTN+BUZZER) | PB10~PB15, PC14, PC15, PC0~PC4, PC5      |
| **하측 우**   | 외부 제어 커넥터 (D-SUB 25P)                                   | PA15, PC10~PC12, PD2                     |
| **우측**      | HRTIM PWM → GaN 드라이버                                       | PA8, PA9                                 |
| **우측**      | 알람 릴레이 5개 + LC 릴레이 4개                                | PA10,PA11,PA12,PB3,PB4 / PC6,PC7,PC8,PC9 |
...
- **하드웨어 정책**: FREQ 버튼 삭제(전면 버튼 미사용).
- **진입**: 통신 명령(Modbus) 또는 내부 서비스 루틴으로만 진입.
- **기능**:
  - 채널 번호 선택 (Ch0~Ch6)
  - 채널 변경 확정/저장
...
**방법 C(74HCT574 ×2)**: 공유 6비트 DATA 버스(PB10~PB15) + CLK_LCD(PC14) / CLK_LED(PC15) 래치 제어.
...
void LCD_HW_LatchLCD(void);                          // CLK_LCD(PC14) 상승엣지 → 74HCT574 #1 래치
...
| 주파수 표시    | `FREQ: 1750kHz ` | `Channel: 5     ` | 통신/내부 서비스 루틴 진입 시 |
...
### 9. 자동 공진 탐색 (`megasonic_ctrl` 내 `ScanResonance()`)

**설계 목적**: 트랜스듀서의 C₀(정전 용량)를 사전에 알 수 없으므로, 주파수 × L 조합을 자동 스윕하여
VSWR 최소점(직렬 공진점)을 탐색한다. 탐색 완료 후 해당 설정으로 고정 발진한다.

**PZT 트랜스듀서 참조 사양**:

| 사이즈    | 두께 공진 주파수 | 정전 용량 C₀ | 해당 채널 |
| --------- | ---------------- | ------------ | --------- |
| 14×15×1mm | **2.0 MHz**      | ~3.2 nF      | **Ch6**   |
| 14×15×2mm | ~1.0 MHz         | ~1.6 nF      | Ch2       |
| 14×15×4mm | ~0.5 MHz         | ~0.8 nF      | Ch0       |

> 14×15×1mm PZT가 현장에서 가장 많이 사용되며, Ch6(2MHz)에 최적 대응한다.

**탐색 알고리즘**:

```c
// 탐색 수행 전 발진 정지 + 1차 탐색: 저전력(RESCAN_POWER_01W=0.15W) 전류 기반
// 2차 정밀 VSWR: RESCAN_POWER_VSWR(0.3W) 또는 RESCAN_POWER_VSWR_LF(0.5W, 500~700kHz)
for (ch = 0; ch <= 6; ch++) {                    // 주파수 7채널
    HRTIM_SetChannel(ch);
    HAL_Delay(RESCAN_FREQ_SETTLE_MS);            // 100ms 안정화
    for (l = 0; l <= 15; l++) {                  // LC 조합 16가지
        MegasonicCtrl_SetLCRelay(l);             // 4개 릴레이 설정
        HAL_Delay(RESCAN_RELAY_SETTLE_MS);       // 20ms 릴레이 확정
        ADC_Sample(RESCAN_SAMPLE_COUNT);         // 10회 평균
        vswr = CalcVSWR(fwd_adc, rev_adc);       // (fwd+rev)/(fwd-rev)×100
        if (vswr < best.vswr_x100)
            best = {ch, l, vswr, fwd_adc, rev_adc, true};
        LCD_ShowProgress(ch, l);                 // 탐색 진행률 표시
    }
}
// 탐색 완료: 최적점 적용 + Flash 저장 + 소프트 파워업
```
...
L_total = L_BASE + (bit0?0.69:0) + (bit1?2.2:0) + (bit2?4.7:0) + (bit3?6.8:0)

비트(L4·L3·L2·L1) | L_total (µH) | f @ C=2.4nF  | f @ C=1.55nF
0000               |  1.94        | 2.33 MHz     | 2.90 MHz
0001               |  2.63        | 2.00 MHz ★   | 2.56 MHz     ← 2MHz/2.4nF PZT 목표
0010               |  4.14        | 1.60 MHz     | 2.04 MHz
0011               |  4.83        | 1.48 MHz     | 1.89 MHz
0100               |  6.64        | 1.26 MHz     | 1.61 MHz
0101               |  7.33        | 1.20 MHz     | 1.53 MHz
0110               |  8.84        | 1.09 MHz     | 1.39 MHz
0111               |  9.53        | 1.05 MHz     | 1.34 MHz
1000               |  8.74        | 1.10 MHz     | 1.40 MHz
1001               |  9.43        | 1.05 MHz     | 1.35 MHz
1010               | 10.94        | 0.97 MHz     | 1.24 MHz
1011               | 11.63        | 0.94 MHz     | 1.20 MHz
1100               | 13.44        | 0.87 MHz     | 1.12 MHz
1101               | 14.13        | 0.85 MHz     | 1.09 MHz
1110               | 15.64        | 0.81 MHz     | 1.03 MHz
1111               | 16.33        | 0.79 MHz     | 1.01 MHz ★   ← 1MHz/1.55nF PZT 목표

※ 0110(8.84µH)≈1000(8.74µH), 0111(9.53µH)≈1001(9.43µH) 근접 쌍 존재 → VSWR 스캔 시 자동 선택
```
...
이상적 공진: rev_adc → 0 → VSWR → 1.00 (vswr_x100 = 100)
탐색 완료 조건: vswr_x100 ≤ RESCAN_VSWR_OK_X100 (=150, VSWR ≤ 1.50)
```

**주의사항**:

- 1차 탐색 전력: `RESCAN_POWER_01W (0.15W)` — 전류 기반, 공진 전 과전류 방지
- 2차 VSWR 전력: `RESCAN_POWER_VSWR (0.3W)` 또는 `RESCAN_POWER_VSWR_LF (0.5W)` — 주파수별 자동 조정
- 릴레이 전환 중 순간 임피던스 불일치 발생 → `RESCAN_RELAY_SETTLE_MS` 반드시 대기
- 탐색 결과는 Flash에 저장 → 전원 재투입 시 재탐색 불필요 (단, 트랜스듀서 교체 시 재탐색 필요)
- 공진점 미발견 시(`valid=false`): LCD에 "TUNE FAIL" 표시, 기본 Ch0(500kHz) + L 조합 0으로 동작
...
- STM32G474RET6: **Flash 512 KB, SRAM 128 KB** — HRTIM 6유닛, COMP 7개, OpAmp 4개 내장.
...
- **LCD1602 (74HCT574 방법 C)**: HD44780 초기화 시퀀스 필수 (8비트×3회 → 4비트 전환).
  DATA 버스(PB10~PB15)에 {RS,EN,D4~D7} 셋팅 → CLK_LCD(PC14) 상승엣지로 74HCT574 #1 래치.
...
- **LCD1602 표시 안 됨** → 74HCT574 #1 래치 확인: DATA 버스(PB10~PB15) 셋팅 후 CLK_LCD(PC14) 상승엣지 발생 여부 확인. 74HCT574 VCC=5V인지 확인. LCD VDD=5V 인가 확인. 4비트 초기화 시퀀스(8비트×3 → 4비트 전환) 확인. 대비 조절용 V0 핀 가변저항 확인.
