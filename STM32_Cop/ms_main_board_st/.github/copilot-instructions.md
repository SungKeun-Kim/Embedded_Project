...
HRTIM 고해상도 타이머(184ps 분해능)를 이용한 200kHz~3MHz 메가소닉 PWM 생성,
LCD1602 캐릭터 디스플레이(4비트 병렬 GPIO), 6개 택트 스위치 + 8개 LED 표시등,
10채널 주파수 선택, 8단계 외부 출력 제어, 다중 알람 릴레이 출력,
...
- 보드: 커스텀 보드 LQFP80 (외부 HSE 크리스탈 또는 고정밀 내부 HSI 16MHz 사용)
...
│   ├── led_indicator.h                # LED 표시등 8개 상태 관리
│   ├── adc_control.h                  # ADC 전력/전류/전압 계측
│   ├── alarm.h                        # 알람 관리 (Err1~Err7, 릴레이 출력)
│   ├── ext_control.h                  # 외부 제어 (REMOTE, SENSOR, 8POWER BCD)
...
├── src/
│   ├── main.c                         # 엔트리포인트, 메인 루프
│   ├── system_clock.c                 # HSI/HSE→PLL→170MHz, HRTIM DLL 초기화
...
│   ├── led_indicator.c                # 8개 LED 상태 표시 로직
│   ├── adc_control.c                  # 이동평균 필터, 전력/임피던스 계산
│   ├── alarm.c                        # 알람 조건 판정, 릴레이 제어, 에러 코드 관리
│   ├── ext_control.c                  # REMOTE/SENSOR/8POWER 외부 입력 처리
...
| 표시       | `led_indicator` (8개 LED 상태)                                            | 디스플레이와 LED 분리                  |
...
| **SN74LS624N** | VCO (전압 제어 발진기)            | **HRTIM Timer A**                                                      | HRTIM 5.44GHz 유효 클럭으로 200kHz~3MHz 직접 생성. 외부 VCO 완전 불필요                              |
| **MC145163**   | PLL 주파수 합성기                 | **HRTIM Timer A**                                                      | Period 레지스터 값 변경으로 184ps 분해능 주파수 설정. 외부 PLL 불필요                                |
| **MCP3202**    | 12비트 SPI ADC (2채널)            | **STM32G474 내장 ADC** (5개, 12비트)                                   | ADC1/ADC2 멀티채널+DMA. SPI 버스 제거로 배선 단순화                                                  |
...
| ---------------------------------- | ------------------------------------------------ | ------------------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| **ULN2003A** (ILN2003AD)           | 달링턴 트랜지스터 어레이 (7채널 릴레이 드라이버) | **ULN2003A 유지** 또는 **개별 N-ch MOSFET (2N7002, BSS138)** | 릴레이 5개 + 부저 1개 = 6채널 필요. ULN2003A 1개로 충분. 3.3V MCU 출력으로 직접 구동 가능 |
...
| 부품                    | 역할                              | 권장 사양                                                                  | 비고                                                                                   |
| ----------------------- | --------------------------------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| **GaN FET**             | 하프브리지 전력 스위칭 (MHz 대역) | EPC2019 (200V), EPC2036 (100V) 등 EPC 시리즈                               | 기존 Si MOSFET 대비: 스위칭 2ns(25배 빠름), 역회복 전류 없음, 5V 게이트. MHz 대역 필수 |
| **GaN 게이트 드라이버** | GaN FET 고속 구동                 | **LMG1210** (TI, 하프브리지 전용, 200V) 또는 **UCC27611** (TI, 단채널, 4A) | HRTIM CHA1/CHA2 → 게이트 드라이버 → GaN FET. 데드타임은 HRTIM에서 184ps 정밀 제어      |
| **방향성 커플러**       | 순방향/역방향 RF 전력 분리        | Mini-Circuits ZGBDC20-372HP+ 또는 동급 (200kHz~3MHz 대역)                  | 메가소닉 MHz 대역에서는 저항/CT 직접 측정 불가 → RF 커플러 필수                        |
...
| **RS-485 트랜시버**     | Modbus RTU 반이중 통신            | **MAX3485** 또는 **SP3485** (3.3V, 반이중)                                 | 기존 RS-422(전이중) → RS-485(반이중) 변경. DE/RE 핀 분리 제어 (PA4/PA5)                |
| **LCD1602**             | 캐릭터 디스플레이 (2행×16칸럼)    | HD44780 호환 LCD1602 모듈                                                  | 4비트 병렬 GPIO (PD0~PD2 + PB12,PB13,PB15). 배경 조명 LED 별도 전원 필요               |
| **릴레이 모듈**         | 알람 출력 5채널                   | DC 5V 코일, 접점 AC100V/DC100V 0.3A                                        | GO, OPERATE, LOW, HIGH, TRANSDUCER 출력용                                              |
...
| 드라이버 IC             | ULN2003A ×1                                          | **ULN2003A ×1 (유지)**                                              |
...
- **HRTIM Timer A**: 200kHz~3MHz 상보 PWM 생성 (CHA1 + CHA2)
- **FET 드라이버**: GaN FET 대응 게이트 드라이버 (LMG1210, UCC27611 등)
- **전력 검출**: 방향성 커플러(Directional Coupler) + RF 전력 검출기(AD8318, LTC5507 등)
- **LCD1602 디스플레이**: HD44780 LCD1602 캐릭터 LCD (2행×16칸럼, 4비트 병렬 GPIO 제어)
- **RS-485 트랜시버**: MAX3485, SP3485 (3.3V 호환)
- **알람 릴레이**: 5개 릴레이 출력 (LOW/HIGH/GO/OPERATE/TRANSDUCER)
- **외부 제어**: REMOTE, SENSOR(강제 정지), 8POWER BCD(3비트 + COM)
...
#### LCD1602 캐릭터 디스플레이 (4비트 병렬 GPIO)

| 핀   | 포트        | 기능                                       |
| ---- | ----------- | ------------------------------------------ |
| PD0  | GPIO_Output | LCD_RS (Register Select: 0=명령, 1=데이터) |
| PD1  | GPIO_Output | LCD_EN (Enable 펄스)                       |
| PD2  | GPIO_Output | LCD_D4 (데이터 비트 4)                     |
| PB12 | GPIO_Output | LCD_D5 (데이터 비트 5) — LQFP80 재배치     |
| PB13 | GPIO_Output | LCD_D6 (데이터 비트 6) — LQFP80 재배치     |
| PB15 | GPIO_Output | LCD_D7 (데이터 비트 7) — LQFP80 재배치     |

> HD44780 호환 LCD1602, 4비트 모드. 상위 니블 → 하위 니블 순서로 2회 니블 전송.
> LQFP80에서 PD3~PD5 미존재로 D5/D6/D7을 PB12/PB13/PB15로 재배치.
> DWT 사이클 카운터를 이용한 마이크로초 지연으로 정밀 타이밍 제어.
> R/W 핀은 GND 고정 (쓰기 전용, 1핀 절약).

#### 택트 스위치 (6개, 내부 풀업, Active LOW)
...
| 핀   | 포트        | 기능                                             |
| ---- | ----------- | ------------------------------------------------ |
| PB0  | GPIO_Output | LED_NORMAL (NORMAL 모드 표시)                    |
| PB1  | GPIO_Output | LED_HL_SET (H/L 알람 설정 모드 표시)             |
| PB2  | GPIO_Output | LED_8POWER (8단계 출력 설정 모드 표시)           |
| PB10 | GPIO_Output | LED_REMOTE (REMOTE 동작 중 표시, 발진 중 깜빡임) |
| PB11 | GPIO_Output | LED_EXT (EXT 외부제어 모드 표시)                 |
| PB14 | GPIO_Output | LED_RX (RS-485 데이터 수신 시 점멸)              |
| PC8  | GPIO_Output | LED_W (디스플레이가 출력 와트값 표시 중)         |
| PC9  | GPIO_Output | LED_kHz (디스플레이가 주파수값 표시 중)          |
...
| PA4 | GPIO_Output | RS-485 DE (Driver Enable, HIGH=송신 활성화)   |
| PA5 | GPIO_Output | RS-485 /RE (Receiver Enable, LOW=수신 활성화) |
...
| 핀  | 포트     | 기능                                                                  |
| --- | -------- | --------------------------------------------------------------------- |
| PA0 | ADC1_IN1 | 순방향 전력 검출 (Forward Power, 방향성 커플러 → RF 검출기 DC 출력)   |
| PA1 | ADC1_IN2 | 역방향 전력 검출 (Reflected Power, 방향성 커플러 → RF 검출기 DC 출력) |
| PA6 | ADC2_IN3 | 출력 전류 측정 (CT 또는 저항 센싱, DC 변환 후)                        |
| PA7 | ADC2_IN4 | 출력 전압 측정 (저항 분압 → DC 변환, 임피던스 계산용)                 |

> **전력 측정 원리**: 메가소닉(MHz 대역)에서는 방향성 커플러로 순방향/역방향 RF 전력을 분리한 뒤,
> AD8318, LTC5507 등 RF 전력 검출기 IC로 로그/리니어 DC 전압을 출력받아 ADC로 읽음.
> 표시값: 전력(W, 0.01W 분해능), 전압(V, 0~9단계), 전류(mA, 0~999), 임피던스(Ω, 50~999).
...
| PC7  | GPIO_Input | SENSOR 입력 (SHORT=정상, **OPEN=Err7 강제 정지**)         |
...
| 핀  | 포트        | 기능                                    |
| --- | ----------- | --------------------------------------- |
| PB5 | GPIO_Output | 에러 경고 알람 및 버튼 조작음 부저 출력 |
...
LQFP80 패키지에서 LCD D5~D7을 PB12/PB13/PB15로 재배치 후 남은 여유 핀.

| 핀       | 비고                             |
| -------- | -------------------------------- |
| PA10     | HRTIM1_CHB1(AF13) 확장 또는 GPIO |
| PB8      | I2C1_SCL(AF4) — EEPROM 확장용    |
| PB9      | I2C1_SDA(AF4) — EEPROM 확장용    |
| PC13     | GPIO 자유 사용                   |
| PC14     | OSC32_IN 또는 GPIO               |
| PC15     | OSC32_OUT 또는 GPIO              |
| PF0, PF1 | HSE 크리스탈 또는 GPIO           |

> **GPIO 총계**: LQFP80에서 약 51개 GPIO 사용 가능 (SWD 2핀 제외 시 ~49개).
> 현재 사용 핀 45개로, **약 6개 여유 핀**을 확보. I2C EEPROM 확장(PB8/PB9) 가능.
> LQFP80에서는 PD3~PD15, PE 포트 전체가 미존재.
...
```text
  ...
  (PA2~PA5)   │                                         │
              └──── Bottom (21~40) ─────────────────────┘
                LED 6ea(PB0~PB2,PB10,PB11,PB14)
                LCD1602(PD0~PD2+PB12,PB13,PB15)
                외부제어(PC6,PC7,PC10~PC12)
```
...
| **하측 중앙** | LCD1602 디스플레이 (4비트 GPIO) | PD0~PD2, PB12, PB13, PB15         |
| **하측 좌**   | LED 표시등 8개                  | PB0~PB2, PB10~PB11, PB14, PC8~PC9 |
| **하측 우**   | 외부 제어 커넥터 (D-SUB 25P)    | PC6, PC7, PC10~PC12               |
| **우측**      | HRTIM PWM → GaN 드라이버        | PA8, PA9                          |
| **우측**      | 알람 릴레이 5개                 | PA11, PA12, PA15, PB3, PB4        |
...
- **LED**: LED_NORMAL 점등, 발진 중 LED_W 점등
...
- **진입**: NORMAL 모드에서 FREQ 버튼 **5초 장기 누름** (LED_kHz 점등)
- **전제**: NORMAL 모드에서만 진입 가능
- **기능**:
  - UP/DOWN 버튼으로 채널 번호 선택 (Ch0~Ch9)
...
HRTIM Timer A를 사용하여 200kHz~3MHz 메가소닉 주파수를 생성한다.
...
**주파수별 Period 및 듀티 분해능:**

| 주파수        | Period (틱) | 듀티 분해능 | 위상 분해능 | 비고               |
| ------------- | ----------- | ----------- | ----------- | ------------------ |
| 200 kHz       | 27,200      | 0.004%      | 0.013°      | 저주파 메가소닉    |
| 285 kHz       | 19,088      | 0.005%      | 0.019°      | Ch0 기본 (매뉴얼)  |
| 430 kHz       | 12,651      | 0.008%      | 0.028°      |                    |
| 750 kHz       | 7,253       | 0.014%      | 0.050°      |                    |
| 1,000 kHz     | 5,440       | 0.018%      | 0.066°      | 1MHz               |
| 1,180 kHz     | 4,610       | 0.022%      | 0.078°      | Ch9 (매뉴얼 최고)  |
| 2,000 kHz     | 2,720       | 0.037%      | 0.132°      | 2MHz               |
| **3,000 kHz** | **1,813**   | **0.055%**  | **0.199°**  | **설계 최대 3MHz** |
...
**10채널 주파수 테이블 (기본값, Flash에 저장/변경 가능):**

| 채널 | 기본 주파수 (kHz) | Period (틱) | 용도               |
| ---- | ----------------- | ----------- | ------------------ |
| Ch 0 | 285.0             | 19,088      | 매뉴얼 기본 주파수 |
| Ch 1 | 430.0             | 12,651      |                    |
| Ch 2 | 580.0             | 9,379       |                    |
| Ch 3 | 750.0             | 7,253       |                    |
| Ch 4 | 950.0             | 5,726       |                    |
| Ch 5 | 1,000.0           | 5,440       | 1MHz 표준          |
| Ch 6 | 1,180.0           | 4,610       | 매뉴얼 최고 주파수 |
| Ch 7 | 1,500.0           | 3,627       |                    |
| Ch 8 | 2,000.0           | 2,720       |                    |
| Ch 9 | 3,000.0           | 1,813       | 설계 최대 주파수   |
...
4비트 병렬 GPIO 모드로 제어하며, DWT 사이클 카운터 기반 마이크로초 지연으로 정밀 타이밍을 제공한다.
`lcd1602`가 상위 API(초기화, 문자열, 커서)를, `lcd1602_hw`가 하위 GPIO 니블 전송을 담당한다.
...
void LCD_HW_PulseEnable(void);                       // Enable 펄스 생성
void LCD_HW_WriteNibble(uint8_t nibble);             // 니블(4비트) + Enable 펄스
void LCD_HW_WriteByte(uint8_t data, uint8_t is_data); // 바이트 전송 (2회 니블, RS 제어)
```
...
| 상태           | 1행 표시 예시    | 2행 표시 예시     | LED_W | LED_kHz | 비고                   |
| -------------- | ---------------- | ----------------- | ----- | ------- | ---------------------- |
| 정지 (설정값)  | `SET:  0.50W   ` | `Ch0  285.0kHz  ` | ON    | OFF     | 설정 출력값 blink      |
| 발진 (실측값)  | `OUT:  0.48W   ` | `Ch0  285.0kHz  ` | ON    | OFF     | 실시간 검출 출력값     |
| 주파수 표시    | `FREQ: 1180kHz ` | `Channel: 6     ` | OFF   | ON      | FREQ 버튼 짧게 누름 시 |
| FREQ SET 모드  | `FREQ SET MODE ` | `Channel: 3     ` | OFF   | OFF     | 채널 선택 중           |
| H/L SET (HIGH) | `HIGH: 0.80W   ` | `SET ALARM HIGH ` | ON    | OFF     | HIGH 알람 설정값       |
| H/L SET (LOW)  | `LOW:  0.20W   ` | `SET ALARM LOW  ` | ON    | OFF     | LOW 알람 설정값        |
| 에러           | `** ERROR **   ` | `Err1: LOW ALARM` | OFF   | OFF     | 에러 코드 표시         |
| EXT ADDR 설정  | `EXT MODE ADDR ` | `Address: 01    ` | OFF   | OFF     | 주소 번호 표시         |
| 8POWER 설정    | `8PWR Step: 3  ` | `OUT:  0.30W    ` | ON    | OFF     | Step 출력값 설정       |
...
| LED_W      | 디스플레이가 출력(W) 표시 중     | —                       |
| LED_kHz    | 디스플레이가 주파수(kHz) 표시 중 | —                       |
...
uint8_t ExtCtrl_GetSensorState(void); // SENSOR 입력 상태 (0=OPEN/에러, 1=SHORT/정상)
uint8_t ExtCtrl_Get8PowerStep(void);  // 현재 BCD 입력에 해당하는 Step 번호 (0~7)
```

> **SENSOR 단자 미사용 시**: D-SUB 25P 커넥터 내부에서 SHORT 처리하여 출고 (매뉴얼 준수).
> SENSOR 단자를 비상 정지 버튼이나 수위 감지 센서에 연결하여 사용 가능.
...
| 0x0003 | R/W | 주파수 채널 번호    | 0~9 (Ch0~Ch9)                        |
...
_그룹 0x50: 주파수 채널 테이블 (10개)_

| 주소          | R/W | 내용           | 단위/범위                 |
| ------------- | --- | -------------- | ------------------------- |
| 0x0050~0x0059 | R/W | Ch0~Ch9 주파수 | ×0.1 kHz (기본값 위 참조) |
...
int main(void)
{
    ...
    while (1) {
        ...
        ExtCtrl_Process();         // REMOTE/SENSOR/8POWER 외부 입력 상태 갱신
...
- **LCD1602 4비트 병렬 GPIO**: HD44780 초기화 시퀀스 필수 (8비트×3회 → 4비트 전환).
  DWT 사이클 카운터 초기화(`CoreDebug->DEMCR` + `DWT->CTRL`) 후 마이크로초 지연 사용.
  R/W 핀은 GND 고정 (쓰기 전용), Enable 폀스 최소 폭 450ns.
  LQFP80에서 PD3~PD5 미존재로 LCD D5/D6/D7을 PB12/PB13/PB15로 재배치.
...
- **LCD1602 표시 안 됨** → 4비트 초기화 시퀀스 확인 (8비트×3 → 4비트 전환). DWT 사이클 카운터 초기화 확인. Enable 폀스 타이밍(450ns) 확인. VDD 5V 인가요 확인 (3.3V GPIO로도 4비트 데이터 전송 가능). 대비 조절용 V0 핀 가변저항 확인. LQFP80에서 LCD D5~D7 핀이 PB12/PB13/PB15로 재배치되었으므로 config.h 확인.
...
- **170 MHz 클럭 설정 실패** → HSI(16 MHz) + PLL 구성에서 PLLM/PLLN/PLLR 값 확인.
