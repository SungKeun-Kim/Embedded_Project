...
volatile boolean triacTriggered = false;     // 트라이악 트리거 상태
volatile int triggerPulseCounter = 0;        // 트리거 펄스 카운터 (100μs 단위)
const int triacPin = 0;                      // PB0 (5번 핀) - 트라이악 제어
const int potPin = 3;                        // PB3 (2번 핀) - 가변저항 (ADC3)
int dimValue;                                // 조광 값, 0 = 최대밝기(100%) ; 158 = 최소밝기(~5%)
...
// 제로크로스 외부 인터럽트 (INT0)
ISR(INT0_vect) {
  // 이미 제로크로스가 감지된 상태면 무시 (디바운싱)
  if (zeroCrossDetected == false) {
